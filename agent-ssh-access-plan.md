...
- **Enforcement**: read-only is enforced at the SSH layer, not just via OS file permissions. Each key's `authorized_keys` entry uses a forced command wrapping `rrsync` in read-only mode (`rrsync -ro /`), plus `restrict` (covers `no-pty,no-agent-forwarding,no-X11-forwarding,no-port-forwarding` in one option). Even if the bot user's file permissions are ever misconfigured to allow writes, the key itself cannot execute anything but a read-only rsync pull.
...
- Cross-project isolation on shared production boxes (see path scope tradeoff above).
